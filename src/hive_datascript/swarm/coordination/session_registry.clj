...
;; Moved from hive-mcp.swarm.datascript.coordination.session-registry.

(ns hive-datascript.swarm.coordination.session-registry
...
(defn get-completed-tasks-this-session
  ...
  (let [c (conn/ensure-conn)
        ...
        all-tasks (d/q '[:find [(pull ?e [*]) ...]
                       ...
                       db)]
    (->> all-tasks
         (hooks/scope-rows {:session-ref session-ref :parent-of parent-of}
